...
fun center(list: MutableList<Double>): MutableList<Double> {
    ...
    for (index in list.indices) list[index] -= meanOfList
    return list
}//end of center
...
fun accumulate(list: MutableList<Int>): MutableList<Int> {
    for (index in 1 until list.size) list[index] += list[index - 1]
    return list
}//end of accumulate
...
fun convert(n: Int, base: Int): List<Int> {
    if (n < 0 || base <= 1) throw Exception("Нужен n >= 0 И base > 1")
...
//end of decimal. with a lot of help from the internet
...
fun russian(n: Int): String {
    ...
    val thousandPart = bundledInThree(n / 1000, thousandUnitList, isThousand) // n / 1000 --> Взять первые три
    val hundredPart = bundledInThree(n % 1000, hundredUnitList, isThousand = false) // n % 1000 --> Взять последние три
